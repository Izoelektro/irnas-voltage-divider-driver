 ...
 */
static int voltage_divider_measure_enable_set(const struct device *dev, int state)
{
...
int voltage_divider_sample(const struct device *dev)
{
	...
	if (err == 0) {
		...
		adc_raw_to_millivolts(adc_ref_internal(data->adc), data->adc_cfg.gain,
		...
		if (cfg->output_ohm != 0) {
			...
			LOG_DBG("voltage_divider: raw %u ~ %u mV => %d mV", data->raw, val, err);
		} else {
			err = val;
			LOG_DBG("raw %u ~ %u mV\n", data->raw, val);
		}
...
static int divider_setup(const struct device *dev)
{
	...
	if (cfg->has_power_gpio) {
		...
		rc = gpio_pin_configure(data->gpio, gpio_chn_cfg->pin,
					GPIO_OUTPUT_INACTIVE | gpio_chn_cfg->flags);
		if (rc != 0) {
			LOG_ERR("Failed to control feed %s.%u: %d", gpio_chn_cfg->label,
				gpio_chn_cfg->pin, rc);
			return rc;
		}
	}
	...
	*adc_cfg = (struct adc_channel_cfg){
	...
	if (cfg->output_ohm != 0) {
		adc_cfg->input_positive = SAADC_CH_PSELP_PSELP_AnalogInput0 + io_ch_cfg->channel;
	} else {
...
#define VOLTAGE_DIVIDER_INIT(N)                                                                    \
	...
	static const struct divider_config divider_config_##N = {                                  \
		...
		IF_ENABLED(DT_INST_NODE_HAS_PROP(N, power_gpios), (.has_power_gpio = true, ))      \
			IF_ENABLED(DT_INST_NODE_HAS_PROP(N, power_gpios),                          \
				   (.power_gpios =                                                 \
					    {                                                      \
						    DT_INST_GPIO_LABEL(N, power_gpios),            \
						    DT_INST_GPIO_PIN(N, power_gpios),              \
						    DT_INST_GPIO_FLAGS(N, power_gpios),            \
					    }, ))                                                  \
